//House loyalty + Senate loyalty tables
var members;
...
function loyalty(members) {
  ...
  var topLoy = document.getElementById("topLoy");
  ...
  for (var i = topTen; i < sortMembers.length; i++) {
    if (
      topMembers[topMembers.length - 1].votes_with_party_pct ==
      sortMembers[i].votes_with_party_pct
    ) {
      topMembers.push(sortMembers[i]);
    }
  }
  ...
  for (var i = 0; i < topMembers.length; i++) {
    ...
    nvotes.innerHTML = sortMembers[i].total_votes;

    pvotes = document.createElement("td");
    pvotes.innerHTML = sortMembers[i].votes_with_party_pct + "%";

    row.append(names, nvotes, pvotes);

    topLoy.appendChild(row);
  }

  //Bottom 10%

  var bottLoy = document.getElementById("bottLoy");
  ...
  for (var i = topTen; i < sortMembersinv.length; i++) {
    if (
      bottomMembers[bottomMembers.length - 1].votes_with_party_pct ==
      sortMembersinv[i].votes_with_party_pct
    ) {
      bottomMembers.push(sortMembersinv[i]);
    }
  }
